...
public class ArchitecturePkgCreator {
	...
	public static void main(String[] args) {
		...
		if( !theUseCases.isEmpty() ){
			ArchitecturePkgCreator theCreator = new ArchitecturePkgCreator( theContext );
			theCreator.createFeatureFunctionPkgs( theUseCases );
		}
	}
	
	public ArchitecturePkgCreator(
	...
	public void createArchitecturePkg(
			IRPPackage underPkg,
		...
		theNewPkg.changeTo( _context.DESIGN_SYNTHESIS_SYSTEM_ARCHTECTURE_PACKAGE );
		
		IRPClass theSystemBlock = theNewPkg.addClass( andFeatureName );
		theSystemBlock.changeTo( _context.SYSTEM_BLOCK );
		
		IRPStructureDiagram ibd = addInternalBlockDiagram( theSystemBlock );
		ibd.openDiagram();
		ibd.highLightElement();
		
		IRPObjectModelDiagram bdd = addBlockDefinitionDiagram( null, theSystemBlock );
		bdd.openDiagram();
		bdd.highLightElement();
		...
		if( withStereotypeIfApplicable instanceof IRPStereotype ) {
			...
			theSystemBlock.addSpecificStereotype( withStereotypeIfApplicable );
		}		
	}
